...
{ 
      ...
      switch(effectCount) {
          case 0:   if (effectValue == HIGH) {
                        ...
                        FillLEDsFromPaletteColors_Sound(startIndex, sparklePixelNum);
                    } else {
                        setAll(0,0,0);
                        FastLED.show();
                        delay(50);
...
void FillLEDsFromPaletteColors_Sound(uint8_t colorIndex, int pixelNum)
{
      if ( fadeCount >= 0 || effectType == 2) {
            ...
            for (int i = 0; i<14; i++) {
                temp += 10;
                setPixel(temp%NUM_LEDS,0,0,0);
            } 
            Serial.println("Effect Sparkle");
       } else if (effectType==0) {
            //Serial.println("Executing Effect Type 0");
            for( int i = 0; i < NUM_LEDS; i++) {
                leds[i] = ColorFromPalette( currentPalette, colorIndex, brightness/2, currentBlending);
                colorIndex += 2;
            }
            FastLED.show();
            FastLED.delay(1000 / UPDATES_PER_SECOND);
...
void changePaletteBasedOnBeat(int aValue, int &sparklePixel)
{
    ...
    */
    ...
    } else if (aValue > avgValue + 4 && avgValue <= avgValue+15 && fadeCount < 50) {
    ...
    } else if (aValue <= avgValue+4 && fadeCount < 0 && sparkleCount < 0){
